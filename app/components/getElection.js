...
export default function GetElection({ darkMode }) { // Add darkMode as a prop
  ...
  async function fetchElection() {
    ...
    try {
      ...
      const result = await contract.getElection(category);
  ...
  return (
    <div className={`max-w-md mx-auto rounded-lg p-4 shadow-lg border border-gray-300 ${darkMode ? "bg-cardDark text-foregroundDark" : "bg-white text-foregroundLight"}`}>
      <h2 className="text-2xl font-bold mb-4 text-primary">Consultar Última Elección</h2>
      <input
        type="text"
